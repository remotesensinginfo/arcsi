...
DEFAULT_ARCSI_S2CLOUDLESS_MODEL = os.path.join(
    install_prefix, "share", "arcsi", S2CLOUDLESS_MODEL_FILENAME
)


def run_fmask_cloud_msk(
    sen2_toa_img,
    sen2_sat_img,
    ...
    print(
        "\tTwater={} Tlow={} Thigh= {} NIR_17={} nonNullCount={}".format(
            Twater, Tlow, Thigh, NIR_17, nonNullCount
        )
    )

    print("FMASK: Cloud layer, pass 2")
    (pass2file, landThreshold) = fmask.fmask.doPotentialCloudSecondPass(
        fmaskFilenames,
        fmaskConfig,
        pass1file,
        Twater,
    ...
    tmp_out_cloud_msk = fmask.fmask.doCloudLayerFinalPass(
        fmaskFilenames,
        fmaskConfig,
        pass1file,
        pass2file,
    ...
    rsgislib.imageutils.gdal_translate(
        tmp_out_cloud_msk, fmask_cloud_out_msk, gdalformat="KEA"
    )
...
def run_pyfmask_shadow_masking(
    ...
    fmaskCloudsImg = os.path.join(
        tmp_base_dir, "{}_pyfmask_clouds_result.kea".format(tmp_base_name)
    )
    fmaskFilenames = fmask.config.FmaskFilenames()
    fmaskFilenames.setTOAReflectanceFile(sen2_toa_img)
    fmaskFilenames.setSaturationMask(sen2_sat_img)
...
def run_s2cloudless(
    ...
    imgs_info = [
        rsgislib.imageutils.ImageBandInfo(
            file_name=s2_tmp_img, name="sen2", bands=[1, 2, 4, 5, 8, 9, 10, 11, 12, 13]
        )
    ]
    ...
    rsgislib.imagemorphology.create_circular_op(morph_close_opt, morph_close_size)

    out_cls_morph_img = os.path.join(
        tmp_dir, "{}_s2cls_tmp_cls_morph.kea".format(basename)
    )
    out_cls_morph_tmp_img = os.path.join(
        tmp_dir, "{}_s2cls_tmp_cls_morph_tmp.kea".format(basename)
    )
    rsgislib.imagemorphology.image_closing(
        out_cls_img,
        out_cls_morph_img,
        out_cls_morph_tmp_img,
        morph_close_opt_file,
        True,
        morph_close_size,
        "KEA",
        rsgislib.TYPE_8UINT,
    ...
    out_cls_img_clumps = os.path.join(
        tmp_dir, "{}_s2cls_tmp_cls_clumps.kea".format(basename)
    )
    out_cls_img_clumps_rmsml = os.path.join(
        tmp_dir, "{}_s2cls_tmp_cls_clumps_rmsml.kea".format(basename)
    )
    rsgislib.segmentation.clump(
        out_cls_morph_img, out_cls_img_clumps, "KEA", False, 0, False
    )
    rsgislib.rastergis.pop_rat_img_stats(out_cls_img_clumps, True, False, True)
    rsgislib.segmentation.rm_small_clumps(
        out_cls_img_clumps, out_cls_img_clumps_rmsml, min_obj_size, "KEA"
    )

    out_cloud_cls_img = os.path.join(
        tmp_dir, "{}_s2cls_tmp_cls_clumps.kea".format(basename)
    )
    rsgislib.imagecalc.image_math(
        out_cls_img_clumps_rmsml,
        out_cloud_cls_img,
        "b1>0?1:0",
        gdalformat,
        rsgislib.TYPE_8UINT,
    ...
    rsgislib.imagemorphology.create_circular_op(morph_dilate_opt, morph_dilate_size)
    rsgislib.imagemorphology.image_dilate(
        out_cloud_cls_img,
        out_cloud_msk,
        morph_dilate_opt_file,
        True,
        morph_dilate_size,
        "KEA",
        rsgislib.TYPE_8UINT,
