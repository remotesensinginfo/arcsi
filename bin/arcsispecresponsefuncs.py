...
class ARCSIResampleSpectralResponseFuncs(object):
    ...
    def run(
        ...
        respFunc = arcsilib.arcsiutils.readSpectralResponseFunc(
            inputFile, seperator, ignoreLines, wvCol, respCol
        )
        print(respFunc)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="arcsispecresponsefuncs.py",
    ...
    # Define the argument for specifying the column for the response function.
    parser.add_argument(
        "--sample", type=float, default="1", help="""The sampling of the output file."""
    )
    # Define the argument which specifies the standard aersol profile to use.
    parser.add_argument(
        "--method",
        type=str,
        default="NearNeighbour",
        choices=["NearNeighbour"],
