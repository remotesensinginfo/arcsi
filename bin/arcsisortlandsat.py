...
class ARCSISortLandsatData(object):
        ...
        if os.path.exists(os.path.join(outFileDIR, fileName)):
            if userInteract:
                ...
                print(
                    "\t File Size: "
                    + str(
                        (os.path.getsize(os.path.join(outFileDIR, fileName)) / 1024)
                        / 1024
                    )
                )
                # Print File
    ...
    def runSort2RowPath(self, inputDir, outputDir, noDIRStruct, userInteract):
        ...
        for inLandsatDIR in inputLandsatDIR:
            lsMTLFile = rsgislib.tools.filetools.find_file(inLandsatDIR, "*MTL.txt")
            ...
            if filePrefix3 == "LM1" or filePrefix4 == "LM01":
                ...
                print("Sensor was not recognised for DIR: " + inLandsatDIR)
...
if __name__ == "__main__":
    ...
    try:
        if args.rowpath:
            arcsiObj.runSort2RowPath(
                args.input, args.output, args.nodirstruct, args.userinteract
            )
        else:
            if args.inputdirs:
                arcsiObj.runFolders(
